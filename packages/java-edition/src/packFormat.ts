...
export async function getProjectPacks(
	...
	for (const projectRoot of projectRoots) {
		try {
			const files = await core.fileUtil.getAllFiles(externals, projectRoot)
			for (const uri of files.filter(isPackMcmeta)) {
			...
			if (!externals.error.isKind(e, 'ENOENT')) {
				// Ignore ENOENT errors as missing project roots should not affect us finding the
				// pack.mcmeta files. https://github.com/SpyglassMC/Spyglass/issues/2034
				throw e
			}
		}
	}
