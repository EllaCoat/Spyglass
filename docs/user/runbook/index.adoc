...
. Restart the editor.
. Open the Command Palette again, type or paste in `spyglassmc.analyzeProject`,
  and press kbd:[Enter].

IMPORTANT: The last step is not optional if you care about the problems of the files that are not
open in the editor. A cache reset only rechecks the files that are open, so the problems of every
other file disappear from the diagnostics panel until `spyglassmc.analyzeProject` recomputes them.
The notification shown when the reset finishes offers the same command as an action; this step is
how you get those problems back if you dismissed it.
...
. Restart the editor.
. Open the Command Palette, type or paste in `spyglassmc.analyzeProject`, and press kbd:[Enter].
  As above, this is what restores the problems of the files that are not open in the editor.
